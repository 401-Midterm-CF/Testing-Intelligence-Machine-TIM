...
// Bring in our MongoodDB
const mongoose = require('mongoose');
mongoose.connect(process.env.MONGODB_URI);

//3rd Party Client Depends
...
const addMember = require('./guild/addMember');
...
client.on('messageCreate', async (message) => {
	...
	if (message.content.toLowerCase().startsWith('!quiz')) {
		...
		message.channel.send(
			`question: ${question} Answers: ${rightAnswer}, ${wrongAnswers}`
		);
		const filterTrivia = (user) => user.author.id === message.author.id;
		console.log(filterTrivia);
		const answer = await message.channel.awaitMessages(filterTrivia, {
			maxMatches: 1,
			time: 10000,
