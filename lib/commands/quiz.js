...
module.exports = {
	...
	async execute(message, args, memberData) {
		...
		const filter = (response) => {
			if (response.user.id === message.author.id) {
				...
				return response.reply({
					content: `You cant use this button, as the game was called ${message.author.username}. Please use !quiz if you would like to play!`,
				});
			}
		};
		...
		collector.on('collect', async (answerSelected) => {
			console.log(
				answerSelected.message.components[0].components[0].label,
				'My Selected answer'
			);
			if (
			...
			console.log(
				'this is the log for the updated answerSelected right before the message.reply',
				answerSelected
			);
		});
